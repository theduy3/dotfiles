...
**Workflow impact:** calling `EnterWorktree` mid-Opus-session → one cache re-warm at the plan→implement boundary; `ExitWorktree` at cleanup → another. Bounded: 2 re-warms per worktree lifecycle (not per turn). Model routing (`/ship`→Sonnet, `/deploy`→Haiku, each a new session) still dominates total cold-start cost. Mitigation: enter the worktree before accumulating large Opus context (smaller prefix re-warms cheaper). The behavior is *correct* (worktree's project context loads) — don't fight it.

**Observe it live:** the cache hit-rate line in `~/.claude/statusline.sh` (line 2) drops the moment the session enters the worktree, then climbs back. See [[statusline-cache-monitoring]].
