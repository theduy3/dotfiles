...
**Overlap to resolve eventually:** four memory layers now coexist — native Claude Code memory (`~/.claude/projects/.../memory/`), the graph-engineering lesson store, `claude-mem` (installed, disabled), and agentmemory. No technical conflict (`deniedMcpServers` blocks `memory`, not `agentmemory`), but agentmemory's `remember`/`recall`/`forget` skills duplicate what the lesson store owns.
