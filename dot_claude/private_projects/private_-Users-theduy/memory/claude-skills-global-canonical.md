...
⚠️ **Five mattpocock skills are `disable-model-invocation: true`** — `to-spec`, `to-tickets`, `triage`, `grill-with-docs`, `implement` (28 of 160 skills carried the flag). They are hidden **by design**, not by `skillListingBudgetFraction` truncation, and pruning does not reveal them. A slash-command's prose saying "run /to-spec" does NOT count as the user typing it, so the Skill tool refuses. The working route is to **read `~/.claude/skills/<name>/SKILL.md` and follow it inline** — which is what `/planmp` now instructs. Upstream is independently converting these to "Call the Skill tool with X".

**Devin CLI sees `/planmp` through a symlink (2026-09-26):** `~/.config/devin/skills/planmp/SKILL.md` → `~/.claude/commands/planmp.md`. Devin CLI imports Claude commands only from a PROJECT `.claude/commands/`, never from global `~/.claude/commands/`, so without the link `/planmp` (and earlier `/f-o`) never loaded in Devin. One file serves both tools — edit `planmp.md`, not the link. `~/.config/devin/` is not chezmoi-managed either.

**`/planmp` is plan-only since 2026-09-26 (operator decision):** spec → grill (APPROVE SPEC) → tickets (APPROVE PLAN) stored as **beans** (epic + child beans, tags `ticket:afk`/`ticket:hitl`, `--blocked-by`), then it prints one self-contained prompt for **any agent fleet lead** (fleet- and harness-agnostic; repo rules/gates are filled in from the repo's own AGENTS.md/docs) and STOPS — it never sends the prompt; the operator copies it. It no longer claims, spawns OMP tabs, reviews or merges. The old OMP orchestration (former steps 6-9) lives in `~/.claude/docs/omp-delegation.md`; full old file at `~/.claude/archive/planmp.md.2026-09-26-omp-orchestrator`. ⚠️ salonx `AGENTS.md` "delegate" section still points to "planmp.md's AFK path" — repoint it to `~/.claude/docs/omp-delegation.md` (planned in the beans-migration Phase 1).

⚠️ **`~/.claude/commands/planmp.md` and `claude-pi.md` are NOT chezmoi-managed** — only `vault-save.md` is. They exist on the Mac alone. This is the `re-add` never DISCOVERS trap: unmanaged files are invisible to the sync, to `chezmoi status`, and to any drift check, so nothing ever reports their absence on the VPSes.
