...
description: "Which plugin/skill system to use per task type — /planmp (Matt Pocock package) is the planning loop owner; ECC + Superpowers are explicit leaf libraries"
metadata:
  node_type: memory
  type: feedback
...
## One planning loop owner — /planmp (updated 2026-09-25)

> `/planmp` (`~/.claude/commands/planmp.md`, ex `/f-o`) is the only planning loop:
> brainstorm → `/to-spec` → `/grill-with-docs` (APPROVE SPEC) → `/to-tickets` (APPROVE PLAN)
> → AFK tickets delegated to OMP/herdr workers, HITL tickets worked live. The former `/s*`
> workflow was retired and removed 2026-09-25. ECC + Superpowers stay ENABLED as
> **explicit-call leaf libraries** — never their workflow loops. See [[spec-plan-tdd-ownership]].

### The loops
- **Planning + delegation:** `/planmp`.
- **Prod:** `/gsd-*` (`gsd-new-project`, `gsd-plan-phase`, `gsd-execute-phase`, `gsd-verify-work`, `gsd-progress`, `gsd-resume-work`, `gsd-workspace`).
- **Skip (rival loops):** `/ecc:plan`, `/ecc:feature-dev`; Superpowers workflow loop.
...
`worktree-required-guard.js` (falsely blocked GSD edits off stale task state) deleted. `worktree-path-guard.js` + `worktree-branch-guard.js` are neutral safety — keep.
