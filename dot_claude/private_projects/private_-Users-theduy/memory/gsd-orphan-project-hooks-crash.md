...
**How to apply:** strip the whole `hooks` key — `jq --indent 2 'del(.hooks)' f > tmp && node -e "JSON.parse(...)" && mv`. Do it for the main checkout AND every worktree settings file. Keep the `Bash(npx gsd-core *)` permission entry (not a hook). Hit on salonx 2026-06-19. Related: [[gsd-reinstall-global]].
